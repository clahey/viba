...
NoteSequenceData TuneLy::sEmptyBar(TimeDelta::sBar);
ChordSequenceData TuneLy::sEmptyChordBar(TimeDelta::sBar);

TunePtr
TuneLy::create()
{
  return TunePtr(new TuneLy);
}
...
bool
TuneLy::Parse(const Glib::ustring& xmlFile)
{
  bool ret;
...
TuneLy::GetNotes(const BarData& bar,
...
TuneLy::GetChords(int bar, bool lastTime)
  const
{
